...
export default function BatchDetailClient({
    ...
    return (
        <div className="max-w-lg mx-auto min-h-screen flex flex-col">
            {/* Header */}
            <div className="bg-white sticky top-0 z-10 border-b border-gray-200 px-3 py-2 shadow-sm">
                ...
                {/* Progress Bar */}
                <div className="flex items-center justify-between text-[10px] font-medium text-gray-600 mb-0.5">
...
function RequestCard({ request, batchId }: { request: Request, batchId: string }) {
    ...
    return (
        <div className={`bg-white rounded-xl shadow-sm border overflow-hidden transition-all duration-300 ${isShipped ? 'border-green-200 bg-green-50/30' : 'border-gray-200'
            }`}>
            {/* 1. Top: Customer Info */}
            <div className="p-2.5 border-b border-gray-100 flex justify-between items-start">
                <div>
                    <div className="font-bold text-gray-900 text-sm flex items-center gap-1.5">
                        {request.recipient_name || '이름 없음'}
                        {isShipped && (
                            <span className="text-[9px] bg-green-100 text-green-700 px-1.5 py-0 rounded-full border border-green-200">
                                발송완료
                            </span>
                        )}
                    </div>
            ...
            <div className="p-2.5 bg-gray-50/50">
                <button
                    ...
                    onClick={copyAddress}
                    className="w-full h-8 flex items-center justify-center gap-1.5 bg-blue-50 hover:bg-blue-100 active:bg-blue-200 text-blue-700 font-bold rounded-lg border border-blue-100 transition-colors shadow-sm text-xs"
                >
                ...
                <div className="mt-1.5 text-[10px] text-gray-400 px-1 truncate">
            ...
            <div className="p-2.5 space-y-2">
                {request.request_items?.map((item) => (
                    <div key={item.id} className="flex gap-2 p-1.5 rounded-lg hover:bg-gray-50 border border-transparent hover:border-gray-100 transition-colors cursor-pointer" onClick={(e) => {
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-0.5 text-[10px] text-gray-500 flex flex-wrap gap-1.5">
                                <span className="bg-gray-100 px-1 py-0 rounded text-gray-600">
                                    {item.user_quantity}개
                                </span>
                                {item.user_selected_option && (
                                    <span className="bg-gray-100 px-1 py-0 rounded text-gray-600 max-w-full truncate">
                                        {item.user_selected_option}
                                    </span>
                                )}
                            </div>
            ...
            <div className="p-2.5 border-t border-gray-100 bg-gray-50">
                {isShipped ? (
                    <div className="flex items-center justify-between bg-white border border-green-200 p-2 rounded-lg shadow-sm">
                        <div className="flex flex-col">
                            <span className="text-[10px] text-green-600 font-bold">운송장 번호</span>
                            <span className="font-mono text-sm font-bold text-gray-900">{trackingNo}</span>
                        </div>
                        <button
                            onClick={() => setIsShipped(false)}
                            className="text-[10px] text-gray-400 underline p-1"
                        >
                            수정
                        </button>
                    </div>
                    ...
                    <div className="flex gap-1.5">
                        <input
                            ...
                            placeholder="운송장 번호"
                            className="flex-1 h-9 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-sm"
                        />
                        <button
                            onClick={handleVerify}
                            disabled={loading || !trackingNo}
                            className="h-9 px-3 bg-gray-900 text-white font-bold rounded-lg shadow-sm hover:bg-gray-800 disabled:bg-gray-300 disabled:cursor-not-allowed whitespace-nowrap text-xs"
                        >
                            {loading ? '...' : '발송'}
                        </button>
                    </div>
